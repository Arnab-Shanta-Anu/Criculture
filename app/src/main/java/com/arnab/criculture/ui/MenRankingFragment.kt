...
class MenRankingFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnTestRankingMen.setOnClickListener {
            viewModel._testRankingMen.observe(viewLifecycleOwner) { it ->
        ...
        binding.btnOdiRankingMen.setOnClickListener {
            viewModel._odiRankingMen.observe(viewLifecycleOwner) { it ->
                val teams = mutableListOf<Team>()
                it.data.forEach { it ->
                    it.team.forEach {
                        teams.add(it)
        ...
        binding.btnT20RankingMen.setOnClickListener {
            viewModel._t20RankingMen.observe(viewLifecycleOwner) { it ->
                val teams = mutableListOf<Team>()
                it.data.forEach { it ->
                    it.team.forEach {
                        teams.add(it)
